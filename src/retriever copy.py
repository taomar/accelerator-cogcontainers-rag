...
import string
from farasa.segmenter import FarasaSegmenter  # Arabic tokenization
...
# Initialize Farasa Segmenter (for Arabic tokenization)
...
# Function to tokenize text for BM25 (Separate methods for Arabic & English)
def tokenize_text(text, language):
    """Tokenizes text differently based on language."""
    text = text.lower().strip()  # Normalize text

    if language == "arabic":
        # Arabic Tokenization using Farasa
...
def search_documents(query, language):
    """Retrieves documents using vector search & BM25 keyword matching, with language-specific BM25 optimization."""
